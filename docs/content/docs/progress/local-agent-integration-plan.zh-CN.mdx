...
```bash
claude mcp add --scope user --transport stdio alien-ai-studio -- npx -y @arm0ne/alien-ai-studio-agent@latest mcp
```
...
- 用户运行 `npx -y @arm0ne/alien-ai-studio-agent@latest` 后，线上画布能显示已连接。
