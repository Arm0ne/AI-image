...
export function checkVersions() {
    ...
    if (!localCodexVersion) {
        ...
        logger.warn(`Bundled Codex ${CODEX_VERSION} does not match local Codex ${localCodexVersion}. Keep both current with: npm install -g @openai/codex@latest && npx -y @arm0ne/alien-ai-studio-agent@latest`);
    }
...
async function checkLatestVersions(localCodexVersion: string) {
    try {
        const [latestAgent, latestCodex] = await Promise.all([
            npmVersion("@arm0ne/alien-ai-studio-agent"),
            npmVersion("@openai/codex"),
        ]);
        if (isOlder(VERSION, latestAgent)) logger.warn(`Update available: Alien AI Studio Agent ${VERSION} -> ${latestAgent}. Run: npx -y @arm0ne/alien-ai-studio-agent@latest`);
        if (isOlder(CODEX_VERSION, latestCodex)) logger.warn(`Update available: bundled Codex ${CODEX_VERSION} -> ${latestCodex}. Upgrade Alien AI Studio Agent with: npx -y @arm0ne/alien-ai-studio-agent@latest`);
