...
+ [优化] 生图工作台按需缩放历史图片缩略图并延迟加载原图，避免大量 4K 历史图片拖慢输入和页面滚动。
...
+ [调整] Alien AI Studio Agent `0.1.1` 将内置 Codex 固定依赖升级到 npm 最新稳定版 `0.154.0`。
+ [修复] 删除当前 Codex 历史会话后自动创建新的空白会话，不再停留在无限初始化状态；删除其他历史会话不再重置当前会话。
