export default {
    meta: {
        title: "Alien AI Studio",
        description: "智能生图与无限画布创作工具",
    },
    theme: { toggle: "切换主题" },
    common: {
        cancel: "取消",
    ...
    modelPlugin: {
        ...
        authoring: {
            intro: "请为 Alien AI Studio 编写一段模型调用脚本。能力类型：{{capability}}。目标模型：{{model}}。",
    ...
    canvas: {
        ...
        sidePanel: {
            canvas: "画布", assets: "资产", resize: "调整左侧面板宽度", elements: "画布元素", select: "选择", searchNodes: "搜索节点", focusNode: "定位到节点", preview: "放大预览", noNodes: "画布暂无节点", clearAll: "取消全选", selected: "已选 {{count}}", exporting: "正在导出选中元素…", exportName: "画布元素-{{count}}个", exported: "已导出 {{count}} 个元素", exportFailed: "导出失败，请重试",
    ...
    config: {
        ...
        localStorage: {
            ...
            description: "查看 Alien AI Studio 在浏览器中保存的数据量，并按对象仓库统计内容体积。",
            ...
            mainDatabase: "Alien AI Studio 主数据",
            records: "{{count}} 条",
            refresh: "刷新统计",
            readFailed: "读取本地存储失败",
    ...
    agent: {
        ...
        connect: { pluginTitle: "方式一：在 Codex 中使用插件", pluginText: "安装 Alien AI Studio Plugin 后，在 Codex 输入“启动创作站”或“启动生图站”。插件会启动本地 Agent、用默认浏览器打开本站并自动连接。", directTitle: "方式二：直接运行 Agent", directText: "不使用 Codex 插件时，在终端运行下面命令，再回到网页里连接或手动填入 Local URL 和 Connect token。", commandCopied: "命令已复制", pluginReminder: "Codex 插件提醒", pluginReminderText: "只有安装 Codex 插件或手动添加 MCP 后，工具列表才会进入 Codex 上下文并增加 token 消耗；仅运行 npx -y @arm0ne/alien-ai-studio-agent@latest 启动本地 Agent 不会安装 MCP。", removePlugin: "移除插件", removeMcp: "移除手动 MCP", copyCommand: "复制命令", title: "连接本地 Agent", description: "按使用场景选择一种连接方式。", webConnection: "网页连接", autoDiscover: "默认自动读取 Local URL 和 Connect token，失败时再手动填写。", disconnect: "断开", connect: "连接", localAddress: "本地地址", urlPlaceholder: "例如 http://127.0.0.1:17371", token: "连接 Token", tokenPlaceholder: "自动发现，或手动填入 Connect token" },
